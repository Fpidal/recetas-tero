...
export default function InsumosPage() {
  ...
  function handleOpenModal(insumo?: InsumoCompleto) {
    if (insumo) {
      ...
      // Mostrar precio del paquete (precio unitario * cantidad)
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // El usuario ingresa precio del paquete, guardamos precio unitario
    const precioPaquete = parseFloat(form.precio)
    ...
    if (editingId) {
      ...
      // Actualizar cantidad_por_paquete por separado (columna nueva)
      if (!error) {
        await supabase
          .from('insumos')
      ...
      // Si hay precio, actualizar (proveedor opcional)
      if (precio > 0) {
      ...
      // Si hay precio, insertar precio inicial (proveedor opcional)
  ...
  const filteredInsumos = insumos
    ...
    .sort((a, b) => {
      ...
      return 0 // mantiene orden por categoría + nombre del query
    })
