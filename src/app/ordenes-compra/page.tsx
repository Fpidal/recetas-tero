...
import { ivaLineaOrden, IVA_POR_DEFECTO } from '@/lib/iva'
...
function calcularTotalConIva(o: OrdenConProveedor): number {
  ...
  if ((o.estado === 'recibida' || o.estado === 'parcialmente_recibida') && facturaActiva?.factura_items?.length) {
    ...
    return facturaActiva.factura_items.reduce((sum, fi) => {
      ...
      const ivaPct = ocItem ? ivaLineaOrden(ocItem) : IVA_POR_DEFECTO
    ...
    const { data, error } = await supabase
      ...
      .select(`
        *,
        ...
        orden_compra_items (insumo_id, vino_id, cantidad, precio_unitario, iva_porcentaje, insumos (iva_porcentaje))
      `)
      .neq('activo', false)
