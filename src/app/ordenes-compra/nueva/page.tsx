...
interface ItemOrden {
  ...
  contenido: number
  /**
   * Cuántos bultos se piden: 1 horma de reggianito, 2 bifes de chorizo.
   * Es para el proveedor, que despacha por pieza. NO entra en el subtotal —
   * ese sale de `cantidad` (los kg) por el precio, como siempre.
   */
...
export default function NuevaOrdenCompraPage() {
  ...
  async function fetchData() {
    ...
    const [proveedoresRes, insumosRes, vinosRes, numero] = await Promise.all([
      ...
      supabase.from('vinos').select('id, bodega, nombre, categoria, cepa, precio_caja, unidades_caja, descuento_porcentaje').eq('activo', true).order('bodega').order('nombre'),
      getNextOCNumber()
    ])
    ...
    <div className="max-w-4xl">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-6 space-y-4 sm:space-y-6">
        ...
        <div className="border-t pt-4 sm:pt-6">
          ...
          <div className="sm:hidden space-y-3 mb-4">
            <div className="grid grid-cols-2 gap-3">
              <Select
                ...
                onChange={(e) => { setFiltroCategoria(e.target.value); setSelectedInsumo(''); setPrecioUnitario(''); setSelectedBodega(''); setPrecioListaVino(''); setDescuentoVino('') }}
              />
          ...
          {/* Desktop: Vinos - 2 filas compactas */}
          {filtroCategoria === 'Vinos' ? (
                ...
                {items.map((item) => (
                  <div key={item.id} className="bg-gray-50 rounded-lg p-3 border">
                    ...
                    <div className="grid grid-cols-4 gap-2 mb-2">
                      <div>
                        <p className="text-[10px] text-gray-500 mb-0.5">Unid.</p>
                        <input
                          type="text"
                          inputMode="decimal"
                          ...
                          className="w-12 rounded border border-gray-300 px-2 py-1 text-sm font-mono"
                        />
                      </div>
                      <div>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {items.map((item) => (
                      <tr key={item.id}>
                        ...
                        {/* Cuántos bultos van. Es lo que el proveedor prepara;
                            el precio se sigue calculando sobre la columna de al
                            lado. Ver supabase-oc-unidades.sql */}
